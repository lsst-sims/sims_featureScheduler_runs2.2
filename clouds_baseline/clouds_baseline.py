...
def run_sched(surveys, survey_length=365.25, nside=32, fileroot='baseline_', verbose=False,
              extra_info=None, illum_limit=40., cloud_offset_year=0.):
    years = np.round(survey_length/365.25)
    scheduler = Core_scheduler(surveys, nside=nside)
    n_visit_limit = None
    filter_sched = simple_filter_sched(illum_limit=illum_limit)
    observatory = Model_observatory(nside=nside, cloud_db='clouds.db', cloud_offset_year=cloud_offset_year)
...
if __name__ == "__main__":
    ...
    if dbroot is None:
        ...
        fileroot = dbroot
    file_end = 'cloudso%i_ns%i_' % (cloud_offset_year, nslice) + 'v2.2_'
    ...
    run_sched(surveys, survey_length=survey_length, verbose=verbose,
              ...
              nside=nside, illum_limit=illum_limit, cloud_offset_year=cloud_offset_year)
